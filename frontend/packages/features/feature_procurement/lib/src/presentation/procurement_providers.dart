...
final purchaseOrderListProvider = FutureProvider<Page<PurchaseOrderDto>>((
  ref,
) async {
  final result = await ref
      .watch(procurementRepositoryProvider)
...
final purchaseOrderDetailProvider =
    FutureProvider.family<PurchaseOrderDto, int>((ref, id) async {
      final result = await ref
          .watch(procurementRepositoryProvider)
          .purchaseOrder(id);
      return result.getOrThrow();
...
final priceHistoryProvider = FutureProvider.family<Page<PriceHistoryDto>, int>((
  ref,
  productId,
) async {
  final result = await ref
