# WMS Flutter monorepo (mobil)

Satınalma və Anbar İdarəetmə Platformasının **mobil** hissəsi: **pub workspace**
(Dart 3.13 / Flutter 3.47) daxilində bir tətbiq və on paylaşılan paket.
İstifadəçilər: anbardar və filial işçisi.

> **Web burada deyil.** Veb tətbiq (satınalma, menecer, admin, auditor) React 18 +
> TypeScript-dir və [`web/`](../web) qovluğundadır —
> [ADR-013](../docs/adr/ADR-013-web-react-mobile-flutter.md). Ortaq nöqtə yalnız
> `contracts/openapi/`-dir; kod paylaşılmır.
...
└── packages/
    ...
    └── features/
        ...
        ├── feature_procurement/  PR, RFQ, təklif müqayisəsi
        ├── feature_reporting/    dashboard
        ├── feature_consumption/  günlük satış girişi, istehlak nəticəsi (ADR-012)
        └── feature_notifications/ bildiriş siyahısı, oxunmamış sayğacı
```

> Mobil qabıq yalnız `feature_identity`, `feature_inventory`, `feature_consumption`
> (filial hissəsi) və `feature_notifications` route-larını mount edir. Qalan paketlər
> (`feature_master_data`, `feature_procurement`, `feature_reporting`) hələ də build olunur
> və testləri işləyir, lakin ekranları React tərəfə köçürülür — ADR-013-ün «atılan iş»
> bəndi. Yeni mobil iş onların üzərinə qurulmamalıdır.
...
Release APK / IPA build imzalama açarı tələb edir; CI yalnız `--debug` APK yığır
...
dart run melos run test         # bütün paket və tətbiqlərin testləri (266 test)
dart run melos run format       # dart format --set-exit-if-changed .
```

Kökdən: `make mobile-get` · `make mobile-analyze` · `make mobile-test` · `make mobile-run`.
...
`KeycloakAuthMobile` — `flutter_appauth` ilə Authorization Code + PKCE; çıxış
RP-initiated logout-dur (`id_token_hint`, yoxdursa `client_id`).

`wms_auth` paketi platformadan asılı olmayan hissəni saxlayır: `Session`, `JwtParser`,
`SecureTokenStore`, `AuthGuard`, PKCE primitivləri və `SilentRefreshScheduler`.
Web tərəfin OIDC tətbiqi (`openid_client` + `package:web`) ADR-013 ilə silinib —
React app `oidc-client-ts` işlədir.
...
5. Ekran naviqasiyada görünməlidirsə `apps/wms_mobile/lib/router/app_router.dart`
